...
# Importamos las nuevas funciones de Zep
from .zep import zep_client, get_session_history
...
async def invoke(payload: InvokePayload):
    ...
    # 1. El mensaje ya viene limpio desde el webhook
    ...
    initial_input = GlobalState(
        messages=[Message(role="human", content=user_input)],
    ...
    ai_response_content = ""
    if final_state.get("messages") and isinstance(final_state["messages"][-1], Message):
